# Load package
# Supressing messages that were causing errors when executed in javascript
...
nodesWithBetween = as.integer( floor(nrow(nodeList)/10) )
nodeList2 <- nodeList[order(nodeList$nodeBetweenness),]
nocolor <- rep(3, nrow(nodeList)-nodesWithBetween)
color <- rep(1,nodesWithBetween)
...
# Plot network. Not when calling this script from a REST API
#D3_network_LM 

# Save network as html file
networkD3::saveNetwork(D3_network_LM, "TreeResponse.html", selfcontained = TRUE)
